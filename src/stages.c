...
/*
			if(isFunc <= 0 && isLibFunc(word)) isFunc = 1;
			
			if(isFunc > 0){
				memset(bigWord, '\0', sizeof(bigWord));
				strcpy(bigWord, word);
				for(int i = strlen(bigWord); i < sizeof(bigWord); i++) bigWord[i] = '\0';

				fseek(funcEnvBuf, 0, SEEK_SET);
			}
*/
			if(tableID != -1){
				fseek(origin, -1, SEEK_CUR);
