...
static void messages(int flag){
	...
	if(flag == 2){
		pout(32,
			...
			" ",
			"[!] Rules [!]",
			"1. Functions with name prefixed by \"LIB_\" (in declarations and",
			"   calls) will be add to library table.",
			"2. Variables and tables with the prefix \"local\" (or \"_G.\") in",
			"   the beginning line will not be compacted.",
