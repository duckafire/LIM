...
bool is_identifier(char c, char **tmp){
	...
	if(table_key != NULL){
		if(is_from_lua(ident, CKIA_LUA_STD_TABLE)){
			code_to_pverbose = 0;
			treat_standard(ident, table_key, &(lim.env_buf.table_from_lua));

		}else if(is_from_header(ident, lim.header_partitions.table_list)){
			code_to_pverbose = 1;
			treat_standard(ident, table_key, &(lim.env_buf.table_from_header));
		...
		if(is_from_lua(ident, CKIA_LUA_KW)){
			...
			*tmp = string_copy(ident); // it will be treat in "read_source_file"
			...
			code_to_pverbose = 0;
			treat_standard(ident, NULL, &(lim.env_buf.func_from_lua));

		}else if(is_from_lua(ident, CKIA_LUA_STD_TABLE)){
			code_to_pverbose = 0;
			treat_standard(ident, NULL, &(lim.env_buf.table_from_lua));

		}else if(is_from_header(ident, lim.header_partitions.funct_list)){
			code_to_pverbose = 1;
			treat_standard(ident, NULL, &(lim.env_buf.func_from_header));

		}else if(is_from_header(ident, lim.header_partitions.table_list)){
			code_to_pverbose = 1;
			treat_standard(ident, NULL, &(lim.env_buf.table_from_header));
